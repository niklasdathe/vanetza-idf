...
| GAP-PKI-001 | TS 102 941 client | `VIDF_PKI` (default off): EnrolmentRequest/Response and AuthorizationRequest/Response with proof of possession, ECIES/AES-CCM, request hash and `pskRecipInfo` decryption; host and PSA implementations interoperate | No HTTP/transport, CTL/CRL/ECTL retrieval, butterfly keys or re-enrolment scheduling; no PKI ATS executed (credential storage exists: `credentials.hpp`, file and NVS stores) |
