...
| IF-SEC-001 | Security encapsulation/decapsulation boundary | TS 102 723-8; TS 103 097 and TS 102 941 for security profiles/trust | `security::SecurityEntity` (signing) injected into the router; verification (SN-DECAP with `Success`) not implemented, `VIDF_SECURITY_VERIFY` fails the build (GAP-SEC-001) |
